...
from .special import fduty, interp1d_gpu,xim,fstar
...
class Ion:
    ...
    def __init__(self,z,fesc=0.2,A2byA1=0.1,ktrans=200,alpha=2.0,beta=0.0):
        ...
        self.fesc = fesc
        self.deltaV_interp = np.concatenate([np.linspace(-0.951, 2, 200), np.linspace(2.01, 6, 100)])  # delta_V
        self.deltav_gpu = tf.as_tensor(self.deltaV_interp,device='cuda')
    ...
    ### Source term
    def nion_interp(self,Mv:float,deltav):
        def diff(m,Mv,deltaR):
            return fstar(m)*fduty(m,self.z)*m*self.cosmo.dndmeps(m,Mv,deltaR,self.z)/ self.mH * self.ob0 / self.om0
        return self._nion_trapez(diff,Mv,deltav)*self.qion*self.fesc/1e60
    
    def nion_st(self,z,mv):
        def diff(m):
            return fstar(m)*fduty(m,self.z)*m*self.cosmo.dndmst(m,z)/ self.mH * self.ob0 / self.om0
        x = np.linspace(np.log10(self.M_min),np.log10(mv),1000)
        y = diff(10**x)*10**x*np.log(10)
        return np.trapezoid(y,x)*self.qion*self.fesc/1e60
    
    def nion_ps(self,z,mv):
        def diff(m):
            return fstar(m)*fduty(m,self.z)*m*self.cosmo.dndmps(m,z)/ self.mH * self.ob0 / self.om0
        x = np.linspace(np.log10(self.M_min),np.log10(mv),1000)
        y = diff(10**x)*10**x*np.log(10)
        return np.trapezoid(y,x)*self.qion*self.fesc/1e60
    ...
    def nxi_interp(self, Mv, deltav):
        ...
        return self._nxi_trapez(diff, Mv, deltav)/1e60
    ...
    def nxi_st(self, z):
        ...
        y = diff(10**x)*10**x*np.log(10)
        return np.trapezoid(y,x)/1e60

    ### IGM term
    def n_HI(self,deltaV):
        return ((1/u.cm**3).to(u.Mpc**-3)).value / 1e60 * n_H(deltaV) ### coming number density of hydrogen atoms in cm^-3
    
    def CHII(self,z):
        return 2.9*((1+z)/6)**-1.1

    def dnrec_dz_path(self,deltaV,xHII_Field):
        x_HE = 1.08
    ...
    def _nxi_trapez(self, diff_func, Mv, deltav):
        ...
        integrand = tf.as_tensor(integrand)
